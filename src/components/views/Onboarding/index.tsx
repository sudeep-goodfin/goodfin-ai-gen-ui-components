...
import { ChatBasedOnboarding } from './ChatBasedOnboarding';
import { colors } from './designTokens';

// Onboarding step types - updated to match AI animated flow
...
export const onboardingVariants = [
  { id: 'chat-based', label: 'Chat Flow' },
  { id: 'animated-flow', label: 'AI Animated' },
  { id: 'full-flow', label: 'Full Flow' },
  { id: 'name', label: 'Name' },
  { id: 'phone', label: 'Phone' },
  { id: 'accredited-confirm', label: 'Accreditation' },
 ...
 */
export function OnboardingView({ variant = 'chat-based' }: OnboardingViewProps) {
  const [currentStep, setCurrentStep] = useState<OnboardingStep>(
    variant === 'full-flow' || variant === 'animated-flow' ? 'name' : variant as OnboardingStep
  );
  ...
  // For chat-based variant, render the ChatBasedOnboarding component
  if (variant === 'chat-based') {
    return <ChatBasedOnboarding />;
  }
      ...
      <main className="flex-1 flex flex-col items-center justify-start px-4 py-8 overflow-y-auto">
        ...
        {stepToShow === 'summary' && (
          <SummaryStep
            ...
            onBack={() => handlePrevStep('accredited-confirm')}
            onSubmit={() => {
              setIsLoading(true);
              setTimeout(() => {
                setIsLoading(false);
